...
== Introduction 
We have waited enough for lightening network to solve problem of Bitcoin micropayment. Now its the time for BMPN, give it a try and you will like the idea behind it. This uses layer two blockchain which runs on top intel secure memory technology to keep bitcoin in secure vaults in decentralized nodes and process them in a batch to reduce the cost of transaction.  
